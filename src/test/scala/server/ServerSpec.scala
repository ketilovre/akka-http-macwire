...
import com.ketilovre.config.{ApiParallelismConfig, ServerHostConfig, ServerPortConfig}
...
class ServerSpec extends BaseSpec with AfterAll {
  ...
  "boot" should {
    ...
    "start an http server on a given host and port" in {
      ...
      val server = new Server(new Api(Seq(dummyApi), Seq.empty, 1.taggedWith[ApiParallelismConfig]), host, port)
